...
v0.3.0 提供 Apple Silicon（M 系列）Mac 的 DMG。它內附 Python 3.12.13、應用程式及官方公開發行的 Codex 0.156.1；收件者不需下載原始碼，也不需另裝 Python、uv、Node 或 Codex。這是本機 App 啟動器加瀏覽器介面，AI 分析仍需連網與自己的 ChatGPT 帳號。GPT-6 Astra 是否可用依帳號權限與額度為準。
